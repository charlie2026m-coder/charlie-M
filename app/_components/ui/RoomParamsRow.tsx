...
const RoomParamsRow = ({ attributes, maxPersons, size }: { attributes: string[], maxPersons: number, size: number }) => {
  const t = useTranslations('roomParams');
  ...
  const bedType = isKing ? t('kingSize') : isQueen ? t('queenSize') : t('single');

  
  return (
    <div className='flex items-center gap-2 mb-3 flex-wrap'>
      <span className='text-dark text-sm flex items-center gap-1'> <BsFillPersonFill className='size-5 text-blue' />{maxPersons} {t('max')}</span>
      ...
      {(isBalcony || isTerrace) && <>
        ...
        <span className='text-dark text'>{isBalcony ? t('balcony') : t('terrace')}</span>
      </>}
